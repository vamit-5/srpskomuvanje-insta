...
-----------------------------
1. Bira nasumičan hook iz banke tekstova (content/captions.json) za zadatu
   kategoriju (isti tekstovi kao za Feed - Stories nemaju svoj caption,
   pa CTA link mora biti ispisan NA slici).
2. Traži PRAVU fotografiju preko Pexels API-ja (besplatno) - biramo scene
   gde se lice NE vidi jasno (siluete, atmosfera) da izgleda autentično i
   da izbegnemo pravni rizik.
3. Iseca sliku na format cele Instagram Story (1080x1920) i ispisuje hook
   tekst + fiksnu CTA liniju (srpskomuvanje.rs - link u bio-u) preko slike
   (Pillow) - BEZ emoji.
...
zato Stories NE nose caption iz captions.json - koristimo samo "hook" polje
i dodajemo fiksnu CTA liniju direktno na sliku.
"""
...
def draw_line_centered(draw, line, font, y, width, fill):
...
def add_story_text(image_bytes, hook_text):
    ...
    try:
        hook_font = ImageFont.truetype(FONT_PATH, int(width * 0.07))
    ...
    hook_lines = wrap_text(draw, hook_text, hook_font, max_width)

    hook_line_height = int(hook_font.size * 1.25) if hasattr(hook_font, "size") else 24
    cta_line_height = int(cta_font.size * 1.3) if hasattr(cta_font, "size") else 20
    gap = int(height * 0.02)

    total_height = hook_line_height * len(hook_lines) + gap + cta_line_height
    ...
    for line in hook_lines:
        draw_line_centered(draw, line, hook_font, y, width, (255, 255, 255, 255))
        y += hook_line_height

    y += gap
    draw_line_centered(draw, CTA_TEXT, cta_font, y, width, (255, 210, 90, 255))
