...
publish_feed.py
------------------
Objavljuje običnu Feed sliku na Instagram, koristeći podatke koje je
generate_and_host_image.py pripremio u output/post_content.json.
...
def log(msg):
    print(f"[publish_feed] {msg}", flush=True)
...
def load_post_content():
    with open("output/post_content.json", "r", encoding="utf-8") as f:
        return json.load(f)
...
def main():
    ...
    log(f"Pravim media container za kategoriju '{content['category']}'...")
